...
const ALLOWED_EVENTS = [
  'plant.harvested', 'plant.pest_detected', 'plant.lifecycle_stage_changed',
  'lab_test.completed', 'lab_test.failed',
  'compliance.violation', 'environmental.alert',
  'inventory.low', 'license.expiring', 'waste.recorded'
];
...
async function ensureTable() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS webhooks (
      ...
      user_id INTEGER,
      url TEXT NOT NULL,
      events TEXT[] NOT NULL DEFAULT ARRAY['compliance.violation']::text[],
      secret VARCHAR(255),
...
router.post('/', async (req, res) => {
  try {
    ...
    const incoming = Array.isArray(events) && events.length > 0 ? events : ['compliance.violation'];
...
router.post('/:id/test', async (req, res) => {
  try {
    ...
    const payload = {
      ...
      data: { message: 'Test webhook from AICannabisCultivationCompliance' }
    };
