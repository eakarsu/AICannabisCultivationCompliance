...
export default function AgenticComplianceCopilotGeneratingMetrcSubmisPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-agentic-compliance-copilot-generating-metrc-submis/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Agentic compliance copilot generating Metrc submissions from cultivation events</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-agentic-compliance-copilot-generating-metrc-submis/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
