...
export default function AiEndpointsScaffoldedButNotMountedUnderStanPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/gap-ai-endpoints-scaffolded-but-not-mounted-under-stan/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>AI endpoints scaffolded but not mounted under standard /chat-style routes; UI references exist</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-ai-endpoints-scaffolded-but-not-mounted-under-stan/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
