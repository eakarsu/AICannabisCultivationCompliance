...
export default function NoSeedToSaleBarcodeRfidScanningWorkflowPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/gap-no-seed-to-sale-barcode-rfid-scanning-workflow/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No seed-to-sale barcode/RFID scanning workflow</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-seed-to-sale-barcode-rfid-scanning-workflow/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
