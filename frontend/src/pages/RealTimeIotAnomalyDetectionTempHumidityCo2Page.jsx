...
export default function RealTimeIotAnomalyDetectionTempHumidityCo2Page() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-real-time-iot-anomaly-detection-temp-humidity-co2-/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Real-time IoT anomaly detection (temp, humidity, CO2, light) with auto-correction prompts</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-real-time-iot-anomaly-detection-temp-humidity-co2-/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
